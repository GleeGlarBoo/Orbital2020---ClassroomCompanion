...
public class PhysicsQuizGameManager : MonoBehaviour
{
    // UI and interfaces
    ...
    private static List<PhyQuestion> unansweredQuestions;
    private static int questionCount = 0;       // count and display the question number, will be reset whenever user leaves the quiz


    private PhyQuestion currentQuestion;

    public List<PhyQuestion> answeredQuestions;       // to be used for the summary page
    ...
    private void Start()
    {
       ...
       // Debug.Log(relatedQuestions.Count);
        ...
        {
            unansweredQuestions = selectedQuestions.ToList<PhyQuestion>();
        }
    ...
    public void SetNextQuestionOrEnd()
    {
        ...
        if (questionCount == 5)
        {
            ...
            EndQuiz(3);                                             // CHANGE ACCORDING TO SUBJECT
    ...
    public void EndQuiz(int Index)
    {
        ...
        {
            int quizPassedCounter = PlayerPrefs.GetInt("QuizPassedCounter");            // definitely set at the very start of the game when user played first, set in timeManager during refresh
    ...
    void DisplayQuestionTopic(int questionTopicNum, TextMeshProUGUI topicTextToBeDisplayed)
    {
        ...
        {
            case 0:
                topicTextToBeDisplayed.text = "(Topic: Units and Measurements)";
                break;
            case 1:
                topicTextToBeDisplayed.text = "(Topic: Speed, distance and time)";
                break;
            case 2:
                topicTextToBeDisplayed.text = "(Topic: Forces)";
                break;
            case 3:
                topicTextToBeDisplayed.text = "(Topic: Turning effect of forces)";
                break;
            case 4:
                topicTextToBeDisplayed.text = "(Topic: Mass, weight and density)";
                break;
            case 5:
                topicTextToBeDisplayed.text = "(Topic: Work, energy and power)";
                break;
            case 6:
                topicTextToBeDisplayed.text = "(Topic: Pressure)";
                break;
            default:
