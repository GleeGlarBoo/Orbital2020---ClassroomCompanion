...
public class QuizUnlockEngTerms : MonoBehaviour
{
    ...
    public static QuizUnlockEngTerms instance;
    ...
    private void Update()
    {
        ...
        {
            EL_startButton.interactable = false;
        ...
        else if (selectedTopics[0] != 0 && selectedTopics[1] != 0 && selectedTopics[2] != 0)    // if 3 topics selected, cant choose anymore
        {
            ...
            Term4topic2.interactable = false;

        }
        else
        {
            EL_startButton.interactable = true;         // if selected any1 of the topics, can start quiz already
        }
    }
    ...
    public void onTopicSelection(int index)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            if (selectedTopics[i] == index)
                selectedTopics[i] = 0;
            else if (selectedTopics[i] == 0 && OkayToAdd)             // select an empty slot from the array. Was Able to double add, hence we need the indicator that none of the array element = him
            {      
    ...
    public void EngClearSelectedArray()
    {
        for (int i = 0; i < 3; i++)
        {
            selectedTopics[i] = 0;
    ...
    // to be placed in english start button
    public void EL_OnStartingQuiz()
    {
        ...
        Invoke("LoadEnglishQuiz", 0.05f);   // delay to ensure playerprefs are set first. 
